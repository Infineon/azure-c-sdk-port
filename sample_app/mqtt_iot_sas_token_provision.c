...
/**
 * @file mqtt_iot_sas_token_provision.c
 * Application for storing Device ID and SAS token in secured memory for cypress secured kits.
 */
...
void tfm_test_task( void * arg )
{
    ...
    if(get_len>0)
    {
        TEST_DEBUG(("Device certificate is present at UID %d...!!!!!!\r\n",(int)DEVICE_CERT_UID));
    }

    /* Check Root CA is provisioned or not */
    get_len = 0;
    memset( &get_data, 0x00, sizeof(get_data) );
    TEST_DEBUG(("Checking for Root certificate....\n\r"));
    psa_ps_get(ROOT_CA_UID, 0, sizeof(get_data), get_data, &get_len);
    if(get_len>0)
    {
        TEST_DEBUG(("RootCA is present at UID %d...!!!!!!\r\n", (int)ROOT_CA_UID));
    }
    ...
    {
        TEST_DEBUG(("Provisioning Device ID completed...\r\n"));
        get_len = 0;
        memset( &get_data, 0x00, sizeof(get_data) );
        ...
        {
            get_data[get_len] = '\0';
            TEST_DEBUG(("Retrieved Device ID : %s\r\n", get_data));
        }
        else
        {
